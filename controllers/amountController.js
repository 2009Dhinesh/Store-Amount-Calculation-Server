...
const syncUserTotal = async (userId) => {
  if (!userId) return;
  const amounts = await Amount.find({ 'memberShares.user': userId });
  const total = amounts.reduce((acc, curr) => {
    const share = curr.memberShares.find(s => s.user.toString() === userId.toString());
    return acc + (share ? (share.amount || 0) : 0);
  }, 0);
...
const syncGroupTotal = async (groupId) => {
  if (!groupId) return;
  const amounts = await Amount.find({ groupId });
  const total = amounts.reduce((acc, curr) => acc + (curr.amount || 0), 0);
...
const getAmounts = async (req, res) => {
  try {
    ...
    }).populate('createdBy', 'name').populate('memberShares.user', 'name').sort({ dateTime: -1 });
    res.json(amounts);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
